...
assertEndpointRequiresAuthentication('api.v1.teams.relationships.transactions', 'getJson', ['team' => '1']);

test('user with "read" token can see team transactions relationships', function (): void {
    ...
    $response = $this->actingAs($this->user)
        ->getJson(route('api.v1.teams.relationships.transactions', ['team' => $this->team]))
        ->assertStatus(200)
        ->assertJsonStructure([
            'links' => [
                'self',
...
test('denies access to team transactions relationships without "read" token', function (): void {
