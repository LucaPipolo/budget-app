...
assertEndpointRequiresAuthentication('api.v1.categories.relationships.transactions', 'getJson', ['category' => '1']);

test('user with "read" token can see account transactions relationships', function (): void {
    ...
    $response = $this->actingAs($this->user)
        ->getJson(route('api.v1.categories.relationships.transactions', ['category' => $this->category]))
        ->assertStatus(200)
        ->assertJsonStructure([
            'links' => [
                'self',
...
test('denies access to account transactions relationships without "read" token', function (): void {
