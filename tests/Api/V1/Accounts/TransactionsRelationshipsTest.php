...
assertEndpointRequiresAuthentication('api.v1.accounts.relationships.transactions', 'getJson', ['account' => '1']);
...
test('user with "read" token can see account transactions relationships', function (): void {
    ...
    $response = $this->actingAs($this->user)
        ->getJson(route('api.v1.accounts.relationships.transactions', ['account' => $this->account]))
        ->assertStatus(200)
        ->assertJsonStructure([
            'links' => [
                'self',
